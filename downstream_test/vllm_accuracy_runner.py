...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--response_log_max", type=int, default=0)
...
def main() -> None:
            ...
            with tqdm(total=len(examples), desc="Evaluating") as progress:
                for batch_examples in _generation_microbatches(examples, tokenizer, args):
                    ...
                    for example, output in zip(batch_examples, outputs):
                        ...
                        response = generation.text if generation is not None else ""
                        row = None
                        if response_log_max < 0 or logged_responses < response_log_max:
                            ...
                            if generation is not None:
                                row["finish_reason"] = getattr(generation, "finish_reason", None)
                                row["generated_token_count"] = len(getattr(generation, "token_ids", []) or [])
                                row["first_token_ids"] = list((getattr(generation, "token_ids", []) or [])[:8])
    ...
    metrics = {"num_examples": len(examples), "num_scored": num_scored, "num_unscored": num_unscored}
    if num_scored > 0:
        metrics.update(
            {
                "pass@1": float(num_correct / num_scored),
                "accuracy": float(num_correct / num_scored),
                "mean_score": float(score_sum / num_scored),
