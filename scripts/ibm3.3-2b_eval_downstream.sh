...
# Downstream eval from saved curvature PKLs for IBM Granite 3.3 2B.
...
skip_prune_layer_ids="${SKIP_PRUNE_LAYER_IDS:-39}"
...
# Full downstream suite record:
# mmlu;winogrande;truthfulqa;gsm8k;math500;ifeval
# mmlu_stem,mmlu_social_sciences;winogrande;truthfulqa_mc1,truthfulqa_mc2;gsm8k;math500;ifeval
# hf;hf;hf;local_vllm;local_vllm;vllm
# 5;5;0;8;0;0
# 0.25;1000;500;500;500;500
downstream_suite_benchmarks="${DOWNSTREAM_SUITE_BENCHMARKS:-gsm8k;math500}"
downstream_suite_tasks="${DOWNSTREAM_SUITE_TASKS:-gsm8k;math500}"
downstream_suite_backends="${DOWNSTREAM_SUITE_BACKENDS:-local_vllm;local_vllm}"
downstream_suite_fewshots="${DOWNSTREAM_SUITE_FEWSHOTS:-8;0}"
downstream_suite_limits="${DOWNSTREAM_SUITE_LIMITS:-500;500}"
downstream_num_fewshot="${DOWNSTREAM_NUM_FEWSHOT:-5}"
downstream_apply_chat_template="${DOWNSTREAM_APPLY_CHAT_TEMPLATE:-1}"
...
downstream_local_batch_size="${DOWNSTREAM_LOCAL_BATCH_SIZE:-8}"
downstream_generation_max_batch_tokens="${DOWNSTREAM_GENERATION_MAX_BATCH_TOKENS:-49152}"
downstream_max_prompt_length="${DOWNSTREAM_MAX_PROMPT_LENGTH:-2048}"
downstream_max_new_tokens="${DOWNSTREAM_MAX_NEW_TOKENS:-4096}"
downstream_min_tokens="${DOWNSTREAM_MIN_TOKENS:-0}"
downstream_temperature="${DOWNSTREAM_TEMPERATURE:-1.0}"
...
run_eval_for_prune_ops "gate_proj up_proj" "$compare_dir_root/gate_w/"
